...
# PARAMS
x_a = -1.01  # LEFT Boundary (Å)
x_b = 0.86  # RIGHT Boundary (Å)
...
def test_cond_prob_integral_x_vs_x0():
    ...
    df.to_csv("sp_eval-cond_prob_int_x_vs_x0.csv", sep="\t", header=True, index=False, index_label=False)
    ...
    plt.savefig("sp_eval-cond_prob_int_x_vs_x0.svg")
    plt.show()
...
def test_cond_prob_integral_x_vs_t():
    ...
    df.to_csv("sp_eval-cond_prob_int_x_vs_t.csv", sep="\t", header=True, index=False, index_label=False)
    ...
    plt.legend(loc="upper right")
    plt.savefig("sp_eval-cond_prob_int_x_vs_t.svg")
    plt.show()
...
def test_fpt():
    ...
    df.to_csv("sp_eval-fpt.csv", sep="\t", header=True, index=False, index_label=False)
...
def test_sp():
    ...
    df.to_csv("sp_eval-sp.csv", sep="\t", header=True, index=False, index_label=False)
...
def test_local():
    ...
    plt.legend(loc="upper right")
    plt.savefig("sp_eval-cond_prob.svg")
    plt.show()


def test_int_sp():
    df = pd.read_csv("sp_eval-sp.csv", sep=r"\s+", comment="#")
    x = df["X0"]
    y = df["SP"]
    ...
    df.to_csv("sp_eval-sp_int_test.csv", sep="\t", header=True, index=False, index_label=False)
...
def plot_pmf_re():
    df = pd.read_csv("sp_eval-sp_int_test.csv", sep=r"\s+", comment="#")
    x = df["X0"]
    pmf_re = df["PMF_RE"]
    pmf = _pmf(x)
